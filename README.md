...
# Spleeter Web Application

A web application for Spleeter, an audio source separation tool that extracts vocals, drums, bass, and other instruments from audio files.

## Features

- Upload audio files (MP3, WAV, FLAC, OGG, M4A, WMA)
- Separate audio into stems:
  - 2 stems (vocals and accompaniment)
  - 4 stems (vocals, drums, bass, other)
  - 5 stems (vocals, drums, bass, piano, other)
- Download individual stems
- Automatic uploading of stems to Google Cloud Storage

## Requirements

- Python 3.8+
- FFmpeg
- Google Cloud Storage credentials (for storage functionality)

## Installation

1. Clone this repository:
```bash
git clone https://github.com/yourusername/spleeter-web.git
cd spleeter-web
```
...
```bash
docker build -t spleeter-web .
```

2. Run the container:
```bash
docker run -p 8080:8080 -e GCS_BUCKET_NAME=your-bucket-name -v /path/to/credentials.json:/app/credentials.json -e GOOGLE_APPLICATION_CREDENTIALS=/app/credentials.json spleeter-web
```

## Cloud Deployment (Google Cloud Run)

This application is ready to be deployed to Google Cloud Run:

```bash
gcloud builds submit --tag gcr.io/your-project-id/spleeter-web
gcloud run deploy spleeter-web --image gcr.io/your-project-id/spleeter-web --platform managed --allow-unauthenticated
```
...
Based on [Spleeter](https://github.com/deezer/spleeter) by Deezer Research.
